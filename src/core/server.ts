...
import { getMountPath, getRouteName, mountConfigMiddleware } from './utils/app'
...
export class Zarf<S extends Record<string, any> = {}, M extends Record<string, any> = {}> {
    ...
    private lastAddedVerb?: RouteMethod
    ...
    private register(method: RouteMethod, url: string, ...args: any): Error | undefined {
        ...
        this.routes[method]!.push({
            ...
            options: {
                meta: {
                    name: getRouteName(id, vars, this.config.appName),
                    alias: ''
                }
            },
            middlewares
    ...
    // ADVANCED: ROUTE NAMING/ALIASING
    /**
     * Provides an opportunity to add an alias to the route name of the last created route.
     *
     * @param alias {string}
     */
    as(alias: string) {
        if(alias.trim() !== '' && this.lastAddedVerb && this.routes[this.lastAddedVerb]) {
            const elm = this.routes[this.lastAddedVerb]![this.routes[this.lastAddedVerb!]!.length - 1]
            if(elm) {
                if(elm.options.meta.alias && elm.options.meta.alias.trim() !== '') {
                    console.error(`Your attempt to set an alias for "${elm.id}" to "${alias}" is ignored, as there's already an alias "${elm.options.meta.alias}" set for it.`)
                } else {
                    const routeConfig = this.getRouteConfigByName(alias, this.lastAddedVerb)
                    if(routeConfig) {
                        console.error(`Your attempt to set an alias for "${elm.id}" to "${alias}" is ignored, as there's already a handler with the same alias.`)
                    } else {
                        this.routes[this.lastAddedVerb]![this.routes[this.lastAddedVerb!]!.length - 1].options.meta.alias = alias
                    }
                }
            }
        }
    ...
    private getRouteConfigById(id: string, verb: RouteMethod = 'get'): Route<S> | undefined {
        return this.routes[verb] && this.routes[verb]?.find(routeConfig => routeConfig.id === id)
    }

    private getRouteConfigByName(routeNameOrAlias: string, verb: RouteMethod = 'get'): Route<S> | undefined {
        return this.routes[verb] && this.routes[verb]?.find(routeConfig => routeConfig.options.meta.name === routeNameOrAlias.toLowerCase() || routeConfig.options.meta.alias.toLowerCase() === routeNameOrAlias.toLowerCase())
    }

    /**
     *
     * Gets the path of a route by it's autogenerated name, or the user provided alias
     *
     * @param routeNameOrAlias {string} Name or alias of the route you want a `path` for
     * @param verb {RouteMethod} The `verb`
     * @returns the path registered by the route, with the provided route name
     */
    public path(routeNameOrAlias: string, verb: RouteMethod = 'get'): string | undefined {
        const routeConfig = this.getRouteConfigByName(routeNameOrAlias, verb)
        return routeConfig ? routeConfig.id : undefined
    }
